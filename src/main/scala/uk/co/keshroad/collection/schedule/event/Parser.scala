...
class Parser extends Logging {
  ...
  def parse(html: String): String = {
    ...
    val out = doc >/~ validator(text("h1"), "Internal Server Error")(
      _.contains("500"))
  ...
  def councilParse(html: String): String = {
    ...
    val nextCollectionHeadingText = doc >> extractor(
      nextCollectionDivId + " h4",
      texts)

    // Get both lists of bins.
    val nextCollectionBinDates = doc >> elementList(
      nextCollectionDivId + " ul")

    // Get all of the bins listed for both dates.
    val nextCollectionBinDatesBinsAll = doc >> elementList(
      "#nextCollectionSection h4 ul li")

    // Validate that there is a NextCollection section on the page
    val validateNextCollectionSection =
      doc >/~ validator(elementList(nextCollectionDivId))(_.size == 1) match {
        case Left(s) =>
          println("Failed to find exactly one nextCollection section: " + s)
        case Right(i) =>
          println("Successfully found one NExtCollection setion: " + i)

      }

    // Validate that there are two h5 headers for us to parse.
    val validateNumberOfh5s =
      doc >/~ validator(elementList("h5"))(_.size == 2) match {
...
//        println("---------")
      i =>
        i.tagName match {
          ...
          case "ul" =>
            mapOfItems.put(dateName, "test") //TODO = work out how to replace "test" with a List of Strings which are the text from the children(i.children.map(child => (child.text).toList)))
